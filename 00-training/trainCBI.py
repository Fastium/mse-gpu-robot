...
parser.add_argument('--overfitting-plots-display', dest='plots_display', action='store_true', default=True,
                    help='Display the plots of the overfitting')
parser.add_argument('--overfitting-plots-save', dest='plots_save', action='store_true', default=True,
                    help='Save the plots of the overfitting')
parser.add_argument('--overfitting-values-save', dest='values_save', action='store_true', default=True,
                    help='Save the values in CSV files')
...
def main(args):
    ...
    print(f"\n{args.arch}: training finished!\n")
    print(f"Output files:")

    print(f"\tSaved\tonnx\t\t{args.model_dir}")


    plot(args, top1_list_val, top1_list_train, represented_data='acc@1')
    plot(args, loss_list_val, loss_list_train, represented_data='loss', show_logarithmic=True)

	#cbi
    if args.values_save:
        save_csv(args, loss_list_val, "val_loss")
        save_csv(args, top1_list_val, "val_accuracy")
        save_csv(args, loss_list_train, "train_loss")
        save_csv(args, top1_list_train, "train_accuracy")
        save_csv(args, [epoch], "best_epoch")
        print(f"\tSaved\tCSVs\t\t{args.model_dir}/metrics")

	#cbi
    print("Best Model:")
    print(f"\tEpoch\t{best_epoch}/{args.epochs - 1}")
    print("With Metrics:")
    print(f"\tTrain\tLoss\t\t{loss_list_train[best_epoch]}")
    print(f"\tVal\tLoss\t\t{loss_list_val[best_epoch]}")
    print(f"\tTrain\tAccuracy\t{top1_list_train[best_epoch]}")
    print(f"\tVal\tAccuracy\t{top1_list_val[best_epoch]}\n")
...
def train(train_loader, model, criterion, optimizer, epoch):
	...
	#cbi
    tensorboard.add_scalars('Loss',  {'train': losses.avg}, epoch)
    tensorboard.add_scalars('Accuracy',  {'train': acc.avg}, epoch)

    loss_list_train.append(losses.avg)
    top1_list_train.append(acc.avg)
...
def validate(val_loader, model, criterion, epoch):
	...
	#cbi
    tensorboard.add_scalars('Loss',  {'val': losses.avg}, epoch)
    tensorboard.add_scalars('Accuracy',  {'val': acc.avg}, epoch)

    loss_list_val.append(losses.avg)
    top1_list_val.append(acc.avg)

    return losses.avg, acc.avg

def plot(args, val_data, train_data, represented_data, show_logarithmic=False):
    plt.xlabel('epochs')
    plt.ylabel(represented_data)
    plt.plot(val_data, label='validation', color='b')
    ...
    if args.plots_save:
        ...
        plt.savefig(os.path.join(args.model_dir, f"metrics/{represented_data}.png"))
        print(f"\tSaved\t{represented_data}.png\t{args.model_dir}/metrics")

    if args.plots_display:
        plt.show()

    plt.clf()


def save_csv(args, values, name):
    os.makedirs(os.path.join(args.model_dir, "metrics"), exist_ok=True)

    with open(os.path.join(args.model_dir, f"metrics/{name}.csv"), "w+") as file:
        csv.writer(file).writerows([[value] for value in values])
